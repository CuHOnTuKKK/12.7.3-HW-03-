...
def test_add_new_pet_with_valid_data(name='Пес', animal_type='Барбос', age='3', pet_photo='images/cat1.jpg'):
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Котя", "кот", "2", "images/cat1.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_successful_update_self_pet_info(name='Елка', animal_type='Торт', age=4):
...
def test_get_list_of_pets_with_wrong_no_auth_key(filter='my_pets'):
    ...
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    auth_key['key'] += str(generate_random_string(5))
    status, result = pf.get_list_of_pets(auth_key, filter)
...
def test_successful_delete_self_no_pet():
    ...
    pet_id = my_pets['pets'][0]['id']
    pet_id += str(generate_random_string(5))
...
def test_no_pet():
    ...
    Для этого ищем своего питомца в списке, по рандомному ID в 999 знаков"""
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    id_no = str(generate_random_string(999))
...
# req 6
def test_add_new_pet_with_no_valid_photo(name='123', animal_type='222', age='4', pet_photo='images/cat1.txt'):
...
def test_add_new_pet_with_invalid_data(name='Tony', animal_type='cats',
                                     age =str (generate_random_string(999)), pet_photo='images/cat1.jpg'):
    """Проверяем что можно добавить питомца с некорректными данными (строка возраст из 999 символов)"""
    ...
    # Добавляем питомца
    status, result = pf.add_new_pet(auth_key, pet_photo, name, age, animal_type)
...
# req 8
def test_add_new_pet_twice_with_valid_data(name='КОТ', animal_type='айпод',
    ...
    for i in range(0, 5):
        tip_name = my_pets["pets"][i]["name"]
        tip_id = my_pets["pets"][i]["id"]
        status, result = pf.add_new_pet(auth_key, name, animal_type, age, pet_photo)
        ...
        assert status == 200
        assert tip_name == my_pets["pets"][i]["name"]
